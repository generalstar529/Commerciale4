...
const Profile = () => {
    ...
    const [selectedMenu, setSelectedMenu] = useState({ root: 0, sub: 0 });

    useEffect(async () => {
        let _loggedCompany = JSON.parse(sessionStorage.getItem(SESSION_LOGGED_COMPANY));
        if (!_loggedCompany) {
            window.location.href = "/";
            return;
        }

        setLoggedCompany(_loggedCompany)
    }, []);
